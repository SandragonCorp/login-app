...
// const CAROUSEL_TRANSITION_CLASSES = {
//     [CAROUSEL_EFFECTS.SLIDE] : "transition-left",
//     [CAROUSEL_EFFECTS.FADE] : "transition-opacity",
// }
...
export const Carousel = (config: CarouselProps) => {
    ...
    let carouselWindowLeft = 0;
    if (config.effect === CAROUSEL_EFFECTS.SLIDE) {
        carouselWindowLeft = carouselItemActiveIndex * -100;
    }
    ...
    const onCarouselArrowClick = (e: SyntheticEvent, direction: string) => {
        ...
        if (direction === "right") nextCarouselItemActiveIndex++;
        if (direction === "left") nextCarouselItemActiveIndex--;
    ...
    const showNextCarouselItem = (e: SyntheticEvent, nextCarouselItemActiveIndex: number): void => {
        ...
        if (carouselItemActive && !isCarouselMoving(carousel)) {
            ...
            if (config.effect === CAROUSEL_EFFECTS.SLIDE) {
                showNewCarouselItemBySlide(carouselItemActive, carouselItemNextActive, nextCarouselItemActiveIndex);
            } else if (config.effect === CAROUSEL_EFFECTS.FADE) {
                showNewCarouselItemByFade(carouselItemActive, carouselItemNextActive, nextCarouselItemActiveIndex);
            }
    ...
    return (
        <div className={`carousel h-full w-full overflow-hidden ${config.className ?? ''} ${config.effect}`} data-current-active-index={carouselItemActiveIndex}>
            <div className={`carousel-slide-container h-full w-full transition-left duration-${config.animationDuration}`} style={{left: carouselWindowLeft + "%"}}>
                { React.Children.map(config.children, (child: JSX.Element, index) => preRenderCarouselItem(child, index)) }
            </div>
            <div className={`left-button absolute w-16 h-16 top-1/2 left-0 translate-y-N50Percent cursor-pointer ${isHideLeftButton ? 'hidden' : ''}`} onClick={(e: SyntheticEvent) => onCarouselArrowClick(e, "left")}>
            ...
            <div className={`right-button absolute w-16 h-16 top-1/2 right-0 translate-y-N50Percent cursor-pointer ${isHideRightButton ? 'hidden' : ''}`} onClick={(e: SyntheticEvent) => onCarouselArrowClick(e, "right")}>
                <div className='flex justify-center items-center h-full w-full bg-gray-600/[0.4] hover:bg-gray-600/[0.8] rounded-full'>
...
const showNewCarouselItemBySlide = (carouselItemActive: Element, carouselItemNextActive: Element, nextCarouselItemActiveIndex: number): void => {
    const carouselSlidesContainer = carouselItemActive.parentElement; // .carousel-slide-container

    carouselSlidesContainer!.style.left = nextCarouselItemActiveIndex * -100 + "%";
}

const showNewCarouselItemByFade = (carouselItemActive: Element, carouselItemNextActive: Element, nextCarouselItemActiveIndex: number) => {
    carouselItemActive.classList.add("moving");
